# SAKURA SYSTEM デモ環境 Phase 2-B 実装プラン v4
## 「SAKURA OS デモ版追加」確認書 v4 — Codex 第3回レビュー反映版・実装前ゲート承認待ち

**作成日**: 2026-05-10
**作成者**: Claude (Opus 4.7) — プランニング担当
**対象**: SAKURA-SYSTEM-DEMO **v0.2.0 → v0.3.0**
**ステータス**: 🟡 **Codex 第4回（最終）レビュー待ち・実装前ゲート承認待ち**
**役割分担**: 本書 = Opus 担当 / 実装作業 = Sonnet 担当
**前提**: Phase 2-A（v0.2.0 グループウェア）デプロイ完了済み
**改訂理由**: Codex 第3回レビューで指摘された「具体コード/値の明示提示」6項目を v4 に反映（v1, v2, v3 は残置・本書は新規ファイル）

---

## 0. レビュー観点サマリー（Codex 向け）

本書は v3 の構造を踏襲し、Codex 第3回レビューで指摘された **「v4 で具体コード/値を明示的に提示すれば承認可能」** に応えるべく、各セクションで **コードブロック + OK/NG 対比 + 表形式** を多用して実装値を確定提示する。
...
| **v4** | **2026-05-10** | **Codex 第3回レビュー反映（実装前ゲート承認待ち）** | **iframe allowlist NG/OK コード対比提示（§3.4.4） / sandbox 最終値を「最小限版（基本3属性）」+ 拡張3属性切り分けに変更（§3.4.3） / CSP connect-src 暫定値 + 観測フェーズで最小化（§5.7.1） / localStorage コア5 必須 + 拡張3 split（§6.5.2） / ALLOWED_HOSTS + NG_PATTERNS 二重防御 JS 配列両方提示（§10.8.1） / T-OS-14 / T-OS-15 完全 Playwright spec コード提示（§10.8.4 / §10.8.5） / T-OS-10 補強版コード追加（§10.8.6）** |
...
```
判定: v3 方針 OK / v3 のまま実装 まだ保留 → v4 で具体コード/値を明示提示
      → 実装前ゲート承認 🟡 Codex 第4回（最終）レビュー待ち

v3 → v4 反映項目（Codex 第3回 必須6点・全て具体値の明示提示）:
  1. iframe allowlist 4件 OK/NG コード対比          → §3.4.2 / §3.4.4
  2. sandbox 最終値を最小限版に変更                  → §3.4.3 基本3属性 + 拡張切り分け
  3. CSP connect-src 暫定値 + 観測で最小化           → §5.7.1 Phase 1/2/3 フロー
  4. localStorage コア5 + 拡張3 split                → §6.5.2 (Codex 指定5件と完全一致)
  5. ALLOWED_HOSTS + NG_PATTERNS 両方提示            → §10.8.1 (二重防御 JS 配列)
  6. T-OS-14 / T-OS-15 完全 Playwright spec コード   → §10.8.4 / §10.8.5

採用必須修正チェックリスト（§17）:
  - 第1回（v1→v2）7項目 ✅ 継続対応済
  - 第2回（v2→v3）8項目 ✅ 継続対応済
  - 第3回（v3→v4）6項目 ✅ 全項目対応済（具体値・具体コード・対比表 全て提示）

Playwright テスト項目（§10.8）: 15項目（v3 と同数・但し T-OS-10 / T-OS-14 / T-OS-15 を完全コード化）
...
実装前ゲート承認:
  - v4 で具体コード/値を全て提示済 → Codex 第4回（最終）レビューで承認可否を決定
  - Codex 第4回 OK + ユーザー Q1-Q9 回答（特に Q4 / Q8）→ Sonnet 実装着手
```
